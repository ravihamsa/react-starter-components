...
var stringSorter = function stringSorter(item, sortKey) {
    return item[sortKey].toLowerCase();
};
var nonStringSorter = function nonStringSorter(item, sortKey) {
    return item[sortKey];
};
    ...
    _createClass(PaginationWrapper, [{
        ...
        value: function getSortedRecords(records) {
            ...
            if (sortKey === '' || firstRecord === undefined) {
                ...
                var firstValue = firstRecord[sortKey];

                return _utils._.orderBy(records, [function (item) {
                    return typeof firstValue === 'string' ? stringSorter(item, sortKey) : nonStringSorter(item, sortKey);
                }], [sortOrder]);
