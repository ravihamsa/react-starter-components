...
var propsList = ['active', 'error', 'disabled', 'valid', '__shadowValue', 'value', 'type', 'exposeName', 'exposeSelection'];
    ...
    _createClass(RXFormElement, [{
        ...
        value: function componentWillReceiveProps(newProps) {
            ...
            _lodash2.default.each(propsList, function (prop) {
                if (newProps[prop]) {
                    if (prop === 'value') {
                        _this2.updateValue(newProps[prop], 'update');
                    }
                    _this2.updateProps(newProps[prop], prop);
                }
            });
        }
    }, {
        ...
        value: function componentWillMount() {
            ...
            var groupedProps$ = this.props$ /*.filter(x=>x.type !== '__shadowValue')*/.groupBy(function (x) {
            ...
            this.readInputValue();
            _lodash2.default.each(propsList, function (prop) {
                _this3.updateProps(_this3.props[prop], prop);
            });
        }
    }, {
        key: 'readInputValue',
        ...
        value: function addCommunicationListeners() {
            ...
            setSibling$.subscribe(function (val) {
                _this5.updateValue(val.value, 'update');
            });
        }
    }, {
        ...
        value: function addServerValidationListeners() {
            ...
            if (this.props.serverValidation) {
                (function () {
                    var validateRequest$ = _this6.value$.filter(function (val) {
                        return val.type === 'update';
                    }).debounceTime(400).filter(function () {
                        return _this6.state.valid;
                    });
                    var serverValidation = getServerValidationRule(_this6.props.serverValidation);
                    var setError$ = validateRequest$.flatMap(function (val) {
                        return _rxjs2.default.Observable.fromPromise(_dataLoader2.default.getRequestDef(serverValidation.requestId, serverValidation.getParams(val, _this6.context.elementValueIndex)));
                    }).combineLatest().defaultIfEmpty(null);
                    setError$.subscribe(function (resp) {
                        _this6.updateProps(resp[0], 'error');
                    }, function (resp) {
                        _this6.updateProps(resp[0], 'error');
                    });
                })();
        ...
        value: function addValidationListeners() {
            ...
            var setError$ = validateRequest$.mergeMap(function (val) {
                return _rxjs2.default.Observable.from(_this7.validations).filter(function (rule) {
        ...
        value: function addActiveListeners() {
            ...
            valueChange$.filter(function (value) {
                ...
                return _rxjs2.default.Observable.from(_this8.activeRules).filter(function (rule) {
