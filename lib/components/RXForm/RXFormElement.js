    ...
    _createClass(RXFormElement, [{
        ...
        value: function componentWillMount() {
            ...
            groupedProps$.mergeMap(function (group) {
                ...
                return _this3.context.elementProps$.next(value);
            });

            this.value$.distinctUntilChanged(function (a, b) {
                return a.value === b.value;
            }).takeUntil(this.unmount$).subscribe(function (value) {
                return _this3.context.elementValue$.next(value);
            });
            ...
            _utils._.each(this.getPropToStateList(), function (prop) {
                _this3.updateProps(_this3.state[prop], prop);
            });
        }
    }, {
        ...
        value: function propChangeListeners() {
            ...
            var propChange$ = this.context.elementProps$.filter(function (e) {
                return e.field === _this4.props.name;
            });
            propChange$.takeUntil(this.unmount$).subscribe(function (e) {
                if (_this4.context.elementPropIndex[_this4.props.name]) {
                    _this4.setState(_this4.context.elementPropIndex[_this4.props.name]);
                }
            });
        }
    }, {
        ...
        value: function addServerValidationListeners() {
            ...
            if (this.props.serverValidation) {
                var forceServerValidation$ = this.context.communication$.filter(function (val) {
                    return val.type === 'elementServerValidation' && val.field === _this6.props.name;
                });
                ...
                var validateRequest$ = this.value$.filter(function (val) {
                    ...
                    return _this6.updateProps(true, 'inProgress');
                }).do(function () {
                    return _this6.updateProps(true, 'isDirty');
                }).filter(function (val) {
                    return serverValidation.validateRequest(val, _this6.context.elementValueIndex);
                });
                var setError$ = validateRequest$.mergeMap(function (val) {
                    return _rxutils.Rx.Observable.fromPromise(_dataLoader2.default.getRequestDef(serverValidation.requestId, serverValidation.getParams(val, _this6.context.elementValueIndex)));
                }).combineLatest().defaultIfEmpty(null);
                setError$.takeUntil(this.unmount$).subscribe(function (resp) {
                    _this6.updateProps(resp[0], 'serverError');
                    _this6.updateProps(resp[0] ? false : true, 'serverValid');
                    _this6.updateProps(false, 'inProgress');
                }, function (resp) {
                    _this6.updateProps(resp[0], 'serverError');
                    _this6.updateProps(resp[0] ? false : true, 'serverValid');
                    _this6.updateProps(false, 'inProgress');
        ...
        value: function addValidationListeners() {
            ...
            var forceValidate$ = this.context.communication$.filter(function (val) {
                return val.type === 'validate' && val.field === _this7.props.name;
            });
            var validateRequest$ = this.value$.filter(function (val) {
                return val.type === 'update';
            }).merge(forceValidate$);
            var setError$ = validateRequest$.mergeMap(function (val) {
                return _rxutils.Rx.Observable.from(_this7.validations).filter(function (rule) {
                    return rule.func.call(_this7, rule, val.value) !== true;
                }).take(1).defaultIfEmpty(null);
            });
            setError$.takeUntil(this.unmount$).subscribe(function (rule, val) {
                if (_this7.clearing) {
                    _this7.updateProps(true, 'valid');
                    _this7.updateProps(null, 'error');
                    _this7.clearing = false;
                } else {
                    _this7.updateProps(rule ? false : true, 'valid');
                    _this7.updateProps(rule, 'error');
                }
            });
        }
    }, {
        ...
        value: function addActiveListeners() {
            ...
            .mergeMap(function (value) {
                return _rxutils.Rx.Observable.from(_this8.activeRules).filter(function (rule) {
                    return rule.func.call(_this8, {
                        value: valueIndex[rule.element]
                    }, rule) !== true;
                }).mapTo(false).defaultIfEmpty(true);
            }).takeUntil(this.unmount$).subscribe(function (e) {
                _this8.updateProps(e, 'active');
            });
        }
    }, {
            ...
            .takeUntil(this.unmount$).subscribe(function (value) {
                _utils._.each(groupedPropRules, function (rules, prop) {
                    var propValue = _utils._.reduce(rules, function (memo, rule) {
                        return !memo && rule.func.call(_this9, {
                            value: valueIndex[rule.element]
                        }, rule) === true;
                    }, false);
                    _this9.updateProps(_this9.props.getPropValue(prop, propValue), prop);
                });
            });
        }
    }, {
        ...
        value: function getFormClasses() {
            var classArray = ['form-group', this.props.className];
