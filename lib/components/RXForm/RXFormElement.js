...
var propsList = ['active', 'error', 'disabled', 'valid', 'defaultValue', '__shadowValue'];
...
var RXFormElement = function (_Component) {
    ...
    function RXFormElement(props) {
        ...
        _this._value = _this.props.defaultValue;
    ...
    _createClass(RXFormElement, [{
        ...
        value: function addServerValidationListeners() {
            ...
            if (this.props.serverValidation) {
                (function () {
                    var validateRequest$ = _this4.value$.filter(function (val) {
                        return val.type === 'update';
                    }).debounceTime(400).filter(function () {
                        return _this4.state.valid;
                    });
                    var serverValidation = getServerValidationRule(_this4.props.serverValidation);
                    var setError$ = validateRequest$.flatMap(function (val) {
                        return _rxjs2.default.Observable.fromPromise(_dataLoader2.default.getRequestDef(serverValidation.requestId, serverValidation.getParams(val, _this4.context.elementValueIndex)));
                    }).combineLatest().defaultIfEmpty(null);
                    setError$.subscribe(function (resp) {
                        _this4.updateProps(resp[0], 'error');
                    }, function (resp) {
                        _this4.updateProps(resp[0], 'error');
                    });
                })();
        ...
        value: function addValidationListeners() {
            ...
            var validateRequest$ = this.value$.filter(function (val) {
                return val.type === 'update';
            }).merge(this.context.forceValidate$);
            var setError$ = validateRequest$.mergeMap(function (val) {
                return _rxjs2.default.Observable.from(_this5.validations).filter(function (rule) {
                    return rule.func(rule, val.value) !== true;
                }).take(1).defaultIfEmpty(null);
            });
            setError$.subscribe(function (rule, val) {
                _this5.updateProps(rule ? false : true, 'valid');
                _this5.updateProps(rule, 'error');
            });
        }
    }, {
        ...
        value: function addActiveListeners() {
            ...
            valueChange$.filter(function (value) {
                ...
                return _rxjs2.default.Observable.from(_this6.activeRules).filter(function (rule) {
                    return rule.func({ value: valueIndex[rule.element] }, rule) !== true;
                }).mapTo(false).defaultIfEmpty(true);
            }).subscribe(function (e) {
                _this6.updateProps(e, 'active');
            });
        }
    }, {
        key: 'onChange',
        ...
        value: function getRestProps() {
            var props = _lodash2.default.omit(this.props, 'showLabel', 'debounceTime', 'options', 'helperText', 'active', 'error', 'validations', 'activeRules', 'valid', 'serverValidation');
...
RXFormElement.contextTypes = {
    ...
    forceValidate$: _react.PropTypes.object.isRequired,
