...
var getPropRule = function getPropRule(item) {
    return {
        ...
        prop: item.prop,
        value: item.value,
        valueFunc: item.valueFunc || _utils._.identity,
...
var FormElement = function (_Component) {
    ...
    function FormElement() {
        ...
        _this.change$ = new _rxutils.Rx.Subject();
        _this._changing = false;
        _this.state = {
            errors: []
        };
        _this.validations = _utils._.filter(validations, function (item) {
            return item.element === undefined;
        }).map(function (rule, index) {
            return getRuleValue(rule);
        });

        _this.siblingValidations = _utils._.filter(validations, function (item) {
            return item.element !== undefined;
        }).map(function (rule, index) {
            return getRuleValue(rule);
        });
    ...
    _createClass(FormElement, [{
        ...
        value: function setValue(value, skipValidate) {
            ...
            if (this.props.options) {
                ...
                var selectedOption = multiSelect ? _utils._.filter(this.props.options, function (item) {
                ...
                if (this.props.exposeName && selectedOption) {
                    toSet[name + '_name'] = multiSelect ? _utils._.map(selectedOption, 'name') : selectedOption.name;
                }
            }
        ...
        value: function validateSiblingsOnChange(changed) {
            ...
            var changedKey = _utils._.keys(changed)[0];
        ...
        value: function handlePropRules(changed, fullObjecdt) {
            ...
            var changedKey = _utils._.keys(changed)[0];
            if (toValidateIds.indexOf(changedKey) > -1) {
                var propValue = _utils._.reduce(this.propRules, function (memo, rule) {
...
FormElement.propTypes = {
    ...
    exposeSelection: _propTypes2.default.bool,
    exposeName: _propTypes2.default.bool,
