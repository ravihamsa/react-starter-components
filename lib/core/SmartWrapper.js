            ...
            if (this.checkActiveRules(this.props)) {
                ...
                if (stores) {
                    for (var i = 0; i < stores.length; i++) {
                        ...
                        var filteredProps = _.omit(this.props, NATIVE_PROPS);
                        var params = getParams ? getParams.call(this, filteredProps) : filteredProps;
                ...
                var _loop = function _loop(i) {
                    ...
                    if (propDependency !== undefined) {
                        ...
                        if (newPropValue !== oldPropValue) {
                            var filteredProps = _.omit(newProps, NATIVE_PROPS);
                    ...
                    if (propDependencies !== undefined && propDependencies.length !== undefined && propDependencies.length > 0) {
                        _.each(propDependencies, function (propDependency) {
                            var newPropValue = newProps[propDependency];
                            var oldPropValue = prevProps[propDependency];
                            if (newPropValue !== oldPropValue) {
                                var _filteredProps = _.omit(newProps, NATIVE_PROPS);
                                var _params = getParams ? getParams.call(self, _filteredProps) : _filteredProps;
                                self.addRequest(storeConfig.propKey, storeConfig.requestId, _params);
                            }
                        });
                    }
                    // this.loadStore(storeConfig.propKey, storeConfig.store, getParams.call(this, storeConfig), true)
