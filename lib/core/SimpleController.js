...
var reset = function reset(name, valueMap) {
    var map = undefined._dataIndex[name];
    map = map.merge(valueMap);
    undefined._dataIndex[name] = map;
    undefined.triggerChange();
...
var add = function add(Model, name, valueMap) {
    var list = undefined._dataIndex[name];
    list.push(new Model(_util._.extend({}, valueMap, {
...
var createMap = function createMap(name, Model) {
    ...
    undefined._dataIndex[name] = new Model(_util._.extend({}, valueMap, {
        id: (0, _utils.getUniqueId)()
    }));
    undefined._commandIndex['update' + name] = update.bind(undefined, name);
    undefined._commandIndex['reset' + name] = reset.bind(undefined, name);
};

var createList = function createList(name, Model) {
    undefined._dataIndex[name] = (0, _immutable.List)();
    undefined._commandIndex['updateItem' + name] = updateInList.bind(undefined, name);
    undefined._commandIndex['addItem' + name] = add.bind(undefined, Model, name);
    undefined._commandIndex['removeItem' + name] = remove.bind(undefined, name);
};
