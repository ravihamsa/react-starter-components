...
export default class Dropdown extends SelectionFormElement {
    ...
    render() {
        ...
        const ListItem = this.props.ListItem || SelectableListItem;
        const placeholder = this.props.placeholder || 'Please Enter Text';
        const {valign = 'top'} = this.props;

        let filteredOptions = _.filter(options, item => item.name.toLowerCase().indexOf(this.state.query.toLowerCase()) > -1);
        ...
        return <fieldset className={formClasses}>
            ...
            <div className="form-control drop-down">
                <InlinePopup ref={inlinePopup => this.ref_inlinePopup = inlinePopup}  disabled={this.props.disabled}>
                    <InlineButton>
                        {this.renderButton()}
                    </InlineButton>
                    <InlineBody valign={valign} className="inline-popup-body-fullwidth">
                        <div className="drop-down-body">
                            {this.props.showSearch ? <div className="drop-down-search-container"><input type="text" autoFocus defaultValue={this.state.query} ref={searchBox => this.ref_searchBox = searchBox} onChange={this.onKeyPressHandler} className="drop-down-input" placeholder={placeholder}/> </div> : null}
